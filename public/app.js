...
let vedChatMode = localStorage.getItem("vedchat_mode") || "online";
let offlineContacts = loadJSON("vedchat_offline_contacts", {});
let offlineMessages = loadJSON("vedchat_offline_messages", {});
...
// Private chat history is now server-side in chat_history.json.
// Keep only the currently loaded conversations in memory.
let privateMessages = {};
...
// Remove the old browser private-message cache from older VedChat builds.
try {
    localStorage.removeItem("vedchat_private_messages");
} catch {}

function saveLocal() {
    // Private messages are intentionally NOT written to localStorage.
    // They are stored server-side and loaded from chat_history.json.
...
function handleMessage(message) {
    ...
    switch (message.type) {
        ...
        case "private-history":
            if (message.code) {
                privateMessages[cleanCode(message.code)] =
                ...
                if (
                    ...
                    currentFriend === cleanCode(message.code)
                ) {
        ...
        case "private-history-index":
            privateMessages = {};
            Object.keys(message.chats || {}).forEach(code => {
                const item = message.chats[code];
                if (item) privateMessages[code] = [item];
            });
    ...
    render(`
        ...
        <main>
            ...
            <div class="quick-grid">
                ...
                <button
                    class="quick-card offline-card"
                    onclick="showOfflineVedChat()"
                >
                    <span>📴</span>
...
function requestPrivateHistory(code) {
    code = String(code || "").trim().toUpperCase();
    if (!code) return;

    sendRaw({
        type: "get-private-history",
        code,
        limit: 1000
    });
}

function requestPrivateHistoryIndex() {
    sendRaw({
        type: "get-private-history-index"
    });
}
...
function openPrivateChat(code) {
    ...
    // The server is the source of truth for private chat history.
    privateMessages[currentFriend] = [];
    requestPrivateHistory(currentFriend);
    displayPrivateMessages();
...
function saveOffline() {
    localStorage.setItem(
        "vedchat_offline_contacts",
        JSON.stringify(offlineContacts)
    );
    localStorage.setItem(
        "vedchat_offline_messages",
        JSON.stringify(offlineMessages)
    );
}

function setVedChatMode(mode) {
    vedChatMode = mode === "offline" ? "offline" : "online";
    localStorage.setItem("vedchat_mode", vedChatMode);
}

function bluetoothAvailable() {
    return !!(window.AndroidBluetooth && typeof window.AndroidBluetooth.getPairedDevices === "function");
}

function bluetoothRequest() {
    if (!bluetoothAvailable()) {
        alert("Bluetooth Offline VedChat needs the Android version of VedChat.");
        return;
    }
    window.AndroidBluetooth.requestBluetoothPermissions();
}

function bluetoothDevices() {
    if (!bluetoothAvailable()) {
        alert("Bluetooth is available only in the Android VedChat app.");
        return;
    }
    window.AndroidBluetooth.getPairedDevices();
}

window.onBluetoothDevices = function(json) {
    let devices = [];
    try { devices = JSON.parse(json || "[]"); } catch (_) {}
    if (!devices.length) {
        alert("No paired Bluetooth devices found. Pair the two phones in Android Bluetooth settings first.");
        return;
    }
    const choices = devices.map((d, i) => `${i + 1}. ${d.name || "Unknown device"} (${d.address})`).join("\\n");
    const answer = prompt("Choose a paired Bluetooth device:\\n\\n" + choices + "\\n\\nEnter its number:");
    const index = Number(answer) - 1;
    if (!Number.isInteger(index) || !devices[index]) return;
    const d = devices[index];
    window.AndroidBluetooth.connect(d.address, d.name || "Bluetooth friend");
};

window.onBluetoothConnected = function(name) {
    alert("📡 Connected to " + (name || "Bluetooth device") + "!");
};
...
window.onBluetoothMessage = function(json) {
    let packet;
    try { packet = JSON.parse(json || "{}"); } catch (_) { return; }
    if (!packet || packet.type !== "text") return;

    const code = String(packet.code || packet.from || "BLUETOOTH").toUpperCase();
    if (!offlineContacts[code]) {
        offlineContacts[code] = {
            code,
            name: String(packet.name || "Bluetooth friend").slice(0, 40),
            avatar: ""
        };
        offlineMessages[code] = [];
    }
    if (!offlineMessages[code]) offlineMessages[code] = [];
    offlineMessages[code].push({
        id: cryptoRandomId(),
        from: code,
        sender: String(packet.name || offlineContacts[code].name),
        text: String(packet.text || "").slice(0, 5000),
        image: "",
    ...
    saveOffline();
    if (currentPage === "offline-chat" && currentOfflineContact === code) displayOfflineMessages();
};

function sendBluetoothOfflineMessage(text) {
    if (!bluetoothAvailable()) return false;
    try {
        window.AndroidBluetooth.sendMessage(JSON.stringify({
            type: "text",
            code: myCode || "ME",
            name: myName || "Me",
...
function showOfflineVedChat() {
    setVedChatMode("offline");
    currentPage = "offline";

    const contacts = Object.values(offlineContacts);

    render(`
        <header class="topbar">
            <div>
                <h1>📴 Offline VedChat</h1>
                <small>Messages stay on this device</small>
            </div>
        ...
        <main>
            <div class="offline-banner">
                ...
                <span>No server or internet is required.</span>
            </div>

            <button class="primary-btn" onclick="bluetoothRequest(); bluetoothDevices()">
                📡 Connect Bluetooth Device
            </button>

            <button class="primary-btn" onclick="addOfflineContact()">
                ➕ Add Local Chat
            </button>

            <div class="section-title">Offline chats</div>
            <div class="list">
                ${contacts.length ? contacts.map(c => {
                    const count = (offlineMessages[c.code] || []).length;
                    const last = (offlineMessages[c.code] || []).at(-1);
                    return `
                        <button class="list-item" onclick="openOfflineChat('${escapeHTML(c.code)}')">
                            ${avatarHTML(c.avatar || "", c.name, 46)}
                            <div class="list-main">
                                <strong>${escapeHTML(c.name)}</strong>
                                <small>${escapeHTML(last?.text || (last?.image ? "📷 Image" : "No messages yet"))}</small>
                            </div>
                    ...
                    <div class="empty">
                        <div class="empty-icon">📴</div>
                        <h3>No offline chats</h3>
                        <p>Add a local chat to start messaging without the server.</p>
                    </div>`}
            </div>

            <button class="secondary-btn" onclick="setVedChatMode('online'); showHome()">
                🌐 Back to Online VedChat
            </button>
        </main>
    `);
}

function addOfflineContact() {
    const name = prompt("Name for this offline chat:");
    if (!name || !name.trim()) return;

    let code = prompt("Offline chat code (any unique name/code):", "OFF-" + Math.random().toString(36).slice(2, 8).toUpperCase());
    if (!code || !code.trim()) return;
    code = code.trim().toUpperCase();

    if (offlineContacts[code]) {
        alert("That offline chat code already exists.");
        return;
    }
...
function openOfflineChat(code) {
    const contact = offlineContacts[code];
    if (!contact) return;
    currentOfflineContact = code;
    currentPage = "offline-chat";

    render(`
        <header class="chat-header">
            <button class="back-btn" onclick="showOfflineVedChat()">←</button>
            ${avatarHTML(contact.avatar || "", contact.name, 44)}
            <div class="chat-title">
                <strong>${escapeHTML(contact.name)}</strong>
                <small>📴 Offline chat</small>
            </div>
        </header>
        <main class="chat-page">
            <div id="offlineMessageArea" class="message-area"></div>
            <div class="composer">
                <input id="offlineMessageInput" placeholder="Offline message..." maxlength="5000" autocomplete="off">
                <button onclick="sendOfflineMessage()">➤</button>
                <button class="attach-btn" onclick="pickOfflineImage()">📷</button>
            </div>
        </main>
    `);

    const input = document.getElementById("offlineMessageInput");
    ...
    displayOfflineMessages();
    setTimeout(() => input?.focus(), 100);
}

function sendOfflineMessage(textOverride = null, image = null) {
    const input = document.getElementById("offlineMessageInput");
    const text = textOverride !== null ? textOverride : (input?.value || "").trim();
    if (!text && !image) return;
    if (!currentOfflineContact) return;

    if (text && bluetoothAvailable()) {
        sendBluetoothOfflineMessage(text);
    }

    if (!offlineMessages[currentOfflineContact]) offlineMessages[currentOfflineContact] = [];
    ...
    if (offlineMessages[currentOfflineContact].length > 1000) {
        offlineMessages[currentOfflineContact].splice(0, offlineMessages[currentOfflineContact].length - 1000);
    }
    saveOffline();
...
function pickOfflineImage() {
    ...
    input.onchange = () => {
        ...
        reader.onload = () => sendOfflineMessage("", String(reader.result));
        reader.readAsDataURL(file);
    };
    input.click();
}

function displayOfflineMessages() {
    const area = document.getElementById("offlineMessageArea");
    if (!area) return;
    const messages = offlineMessages[currentOfflineContact] || [];
    area.innerHTML = messages.map(message => `
        <div class="message mine offline-message">
            ${message.image ? `<img class="chat-image" src="${message.image}" alt="Offline image">` : ""}
            ${message.text ? `<div>${escapeHTML(message.text)}</div>` : ""}
            <span class="message-time">${formatTime(message.time)} · offline</span>
        </div>
    `).join("");
...
function showChats() {
    ...
    // Load only the lightweight latest-message index for the Chats screen.
    requestPrivateHistoryIndex();
