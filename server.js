        ...
        ws.on(
            ...
            raw => {
                ...
                if (message.type === "edit-message" || message.type === "delete-for-everyone") {
                    ...
                    const record = getMessageRecord(messageId, info.code, to);
                    if (!record) {
                    ...
                    const recipientSocket = getClient(to);
                    if (recipientSocket) send(recipientSocket, payload);
                    else queueMessageEvent(to, payload);

                    // Update any not-yet-delivered original message too.
                    const queue = pendingMessages[to];
                    if (Array.isArray(queue)) {
                        const item = queue.find(m => m && m.messageId === messageId && m.from === info.code);
                        if (item) {
                    ...
                    // Echo to the sender so both devices use the same event path.
                    send(ws, payload);
                    send(ws, { type: "action-ack", messageId, actionType: payload.type === "message-updated" ? "edit-message" : "delete-for-everyone" });
                    return;
                }
