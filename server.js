...
function getClients(code) {
    ...
    for (const [ws, info] of clients.entries()) {
        if (info.code === code) result.push(ws);
    }
    return result;
}
...
function sendToUser(
    ...
    for (const ws of getClients(code)) {
        send(ws, data);
    }
}
...
function queuePrivateMessage(code, packet) {
    ...
    if (packet && packet.messageId) {
        const alreadyQueued = pendingMessages[code].some(
...
/* =====================================================
   ...
   Optional TURN credentials are supplied through environment variables.
   Without TURN, the app keeps the existing Google STUN servers.
===================================================== */
    ...
    ws => {
        ...
        ws.on(
            ...
            raw => {
                ...
                if (
                    ...
                    if (code && users[code]) {
                        ...
                        if (message.avatar !== undefined) {
                            users[code].avatar = String(message.avatar || "").slice(0, 1000000);
                        }

                        if (!Array.isArray(users[code].friends)) {
                            users[code].friends = [];
                        }

                        // Merge the client's saved friend list. This lets a user's
                        // stable code and relationships survive a server restart.
                        if (Array.isArray(message.friends)) {
                            for (const friend of message.friends) {
                                const friendCode = cleanCode(friend && typeof friend === "object" ? friend.code : friend);
                                if (friendCode && friendCode !== code && !users[code].friends.includes(friendCode)) {
                                    users[code].friends.push(friendCode);
                                }
                            }
                        }
                        ...
                        // IMPORTANT: if the client already has a code, keep it.
                        // The old implementation generated a NEW code whenever the
                        // server's in-memory/disk user record disappeared, which
                        // made old friend codes become "User not found" after a
                        ...
                        users[code] = {
                            code,
                            name,
                            avatar: String(message.avatar || "").slice(0, 1000000),
                            friends: [],
                            createdAt: new Date().toISOString()
                        };

                        if (Array.isArray(message.friends)) {
                            users[code].friends = message.friends
                                .map(friend => cleanCode(friend && typeof friend === "object" ? friend.code : friend))
                                .filter(friendCode => friendCode && friendCode !== code);
                        }
                ...
                if (
                    ...
                    const image = typeof message.image === "string" ? message.image.slice(0, 3000000) : "";

                    // Offline-first delivery: a recipient does not have to be
                    // connected (or even currently present in this server process)
                    // for a message to be accepted. A known/valid VedChat code is
                    // enough to queue it. This prevents a weak connection or a
                    // server reconnect from turning a locally-created message into
                    // a misleading "User not found" error.
                    ...
                    const packet = {
                        ...
                        text,
                        ...(image ? { image } : {}),
                        ...(message.messageId ? { messageId: String(message.messageId).slice(0, 100) } : {}),
                    ...
                    const recipientSockets = getClients(to);
                ...
                if (message.type === "edit-message" || message.type === "delete-for-everyone") {
                    ...
                    if (!userExists(to) || !messageId || isBlocked(info.code, to) || isBlocked(to, info.code)) return;
                    const payload = { type: message.type === "edit-message" ? "message-updated" : "message-deleted", from: info.code, to, messageId };
                    if (message.type === "edit-message") payload.text = cleanText(message.text);
                    sendToUser(to, payload);
                    if (message.type === "delete-for-everyone") removePendingMessage(to, messageId);
                    else {
                        const queue = pendingMessages[to];
                        if (Array.isArray(queue)) {
                            const item = queue.find(m => m.messageId === messageId && m.from === info.code);
                            if (item) { item.text = cleanText(message.text); item.edited = true; savePendingMessages(); }
                        }
                    }
                    send(ws, payload);
                    return;
        ...
        ws.on(
            ...
            () => {
                ...
                // Remove this connection BEFORE checking presence. A user is
                // offline only when their last active connection disappears.
                clients.delete(ws);

                if (user && getClients(info.code).length === 0) {
                    sendToUsers(
                        user.friends || [],
                        {
                            type: "presence",
