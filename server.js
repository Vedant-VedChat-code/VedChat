...
function queuePrivateMessage(code, packet) {
    ...
    if (packet && packet.messageId &&
        pendingMessages[code].some(item => item && item.messageId === packet.messageId)) {
        return;
    }
        ...
        ws.on(
            ...
            raw => {
                ...
                if (
                    ...
                    for (const [socket, socketInfo] of clients.entries()) {
                        if (socketInfo && socketInfo.code === to) {
                            recipientSockets.push(socket);
                        }
                    }

                    if (recipientSockets.length) {
                        recipientSockets.forEach(socket => send(socket, packet));
                    } else {
                        queuePrivateMessage(to, packet);
                    }

                    // Authoritative echo tells the sender that the server
                    // accepted this messageId.
                    send(ws, packet);
                ...
                if (message.type === "owner-set-rank") {
                    ...
                    // Rank is public profile metadata in VedChat. Broadcast the
                    // change to every currently connected client so people who
                    // are chatting with the user (even before a friends-list
                    // refresh) see the new badge immediately. The client also
                    // caches the rank for offline/local UI.
