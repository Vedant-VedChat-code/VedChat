        ...
        ws.on(
            ...
            raw => {
                ...
                if (
                    ...
                    const requestedFriends = Array.isArray(message.friends)
                        ? message.friends
                        : [];
                    ...
                    if (
                        ...
                        requestedFriends.forEach(function(friend) {
                            const friendCode = typeof friend === "string"
                                ? cleanCode(friend)
                                : cleanCode(friend && friend.code);
                            if (
                                friendCode &&
                                /^[A-Z0-9]{10}$/.test(friendCode) &&
                                friendCode !== code &&
                                !users[code].friends.includes(friendCode)
                            ) {
                        ...
                        // Keep a valid existing client code stable across app refreshes
                        // and server restarts. Only generate a new code for a first-time user.
                        if (
                            !code ||
                            !/^[A-Z0-9]{10}$/.test(code)
                        ) {
                        ...
                        users[code] = {
                            ...
                            friends: requestedFriends
                                .map(function(friend) {
                                    if (typeof friend === "string") return cleanCode(friend);
                                    return cleanCode(friend && friend.code);
                                })
                                .filter(function(friendCode, index, array) {
                                    return friendCode &&
                                        /^[A-Z0-9]{10}$/.test(friendCode) &&
                                        friendCode !== code &&
                                        array.indexOf(friendCode) === index;
                                }),
