            ...
            raw => {
                ...
                if (message.type === "edit-message" || message.type === "delete-for-everyone") {
                    ...
                    if (!userExists(to) || !messageId || to === info.code || isBlocked(info.code, to) || isBlocked(to, info.code)) {
                        sendError(ws, "Invalid message action.");
                        return;
                    }
                    ...
                    if (!record && message.originalTime) {
                        record = {
                            ...
                            image: message.originalImage ? String(message.originalImage).slice(0, MAX_IMAGE_DATA) : "",
                            sender: user.name,
                            avatar: user.avatar || "",
                    ...
                    if (!record || record.from !== info.code || record.to !== to) {
                        sendError(ws, "Message not found. Please update VedChat server and try again.");
                        return;
                    }
                    ...
                    if (message.type === "edit-message") {
                        if (record.deleted) {
                            sendError(ws, "A deleted message cannot be edited.");
                            return;
                        }
